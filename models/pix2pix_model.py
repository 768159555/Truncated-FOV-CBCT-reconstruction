...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # summary(self.netG, input_size=(1, 256, 256))

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            # summary(self.netD, input_size=(2, 256, 256))
    ...
    def forward(self):
        ...
        # self.real_A= op_modpT(self.real_A)
        # self.real_A = F.interpolate(self.real_A, size=(384,384), mode="bilinear")     #will change real_A  in backward 
    ...
    #     """Calculate GAN loss for the discriminator"""
    #     # Fake; stop backprop to the generator by detaching fake_B
    #     # self.real_As = F.interpolate(self.real_A, size=(384,384), mode="bilinear")
    ...
    #     # gradient_penalty = networks.cal_gradient_penalty(
    #     #     self.netD, real_AB, fake_AB, self.device, lambda_gp=10.0
    #     # )
    #     # combine loss and calculate gradients
    ...
    #     """Calculate GAN and L1 loss for the generator"""
    #     # First, G(A) should fake the discriminator
    #     # self.real_As1 = F.interpolate(self.real_A, size=(384,384), mode="bilinear"
    #     fake_AB = torch.cat((self.fake_B, self.fake_B), 1)
    #     pred_fake = self.netD(fake_AB)
    #     self.loss_G_GAN = self.criterionGAN(pred_fake, True)
    #     # Second, G(A) = B
    #     self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_L1
